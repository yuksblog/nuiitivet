...
def scroll(
    ...
    *,
    ...
    _visualize_scroll(app, px, py, dx=ddx, dy=ddy, target_info=target_info)
    settle(app)
    return result
    ...
    *,
    ...
    if rect is not None and moved:
        ...
        _visualize_scroll(
            ...
            target_info=result["scrolled_into_view"],
...
def _visualize_scroll(
    ...
    *,
    ...
    """Record a human-only scroll marker (best-effort; never breaks the action).

    An unexplained scroll jump is worse than an unexplained click -- the whole
    screen moves -- so the direction is carried into the marker.
    """
    try:
        from . import action_overlay

        target = target_info.get("key") if isinstance(target_info, dict) else None
        action_overlay.record_scroll(app, px, py, dx=dx, dy=dy, target=target, verb=verb)
    except Exception:

...
def is_foreground(widget: Widget) -> bool:
    ...
    is hidden by ``visible(False)``, when it sits on a navigation route that
    another route covers (covered routes stay mounted — only the top one is
    painted), or when a blocking overlay entry is open above it. Each of those
    means the user cannot act on the widget, so its shortcuts must not fire.
    """
    try:

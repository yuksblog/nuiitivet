...
Note: The `Overlay` core provides only `show_modal()` / `show_modeless()` / `show_light_dismiss()`, while scenario-specific APIs (dialogs, snackbars, etc.) are delegated to subclasses.
...
- Use an API that makes it intuitively clear where things are rendered from a user's perspective.
  - `Overlay.show_modal()` / `Overlay.show_modeless()` / `Overlay.show_light_dismiss()` are always on top.
...
"Dialog/Snackbar/Menu" are conceptual examples; technically, they are treated as overlay entries (containing Routes) generated by `Overlay.show_modal()` / `Overlay.show_modeless()` / `Overlay.show_light_dismiss()`.
...
- `Overlay` provides only generic `show_modal()` / `show_modeless()` / `show_light_dismiss()` and no scenario-specific APIs like `dialog`, `snackbar`, or `sheet`.
- Both APIs accept only `Widget | Route`.
...
`Overlay.show_modal()` / `Overlay.show_modeless()` / `Overlay.show_light_dismiss()` are the minimal core APIs for displaying content on the topmost layer.
...
- `dismiss_on_outside_tap=False` (only `show_modal`)
  - `True`: Closes the overlay when tapping outside the displayed content.
- `barrier_color` (only `show_modal`)
  - Color of the modal barrier.
- `show_modeless`
  - Always transparent and background-interactive.
- `show_light_dismiss`
  - Transparent full-screen hit layer; tapping outside closes the entry and consumes the outside tap.
- `timeout: float | None`
  - Duration in seconds.
...
- `show_modal` always renders with a modal barrier.
- `show_modeless` never renders a visible barrier and always passes input through to layers below.
- `show_light_dismiss` renders an invisible outside-tap layer and closes on outside tap.
...
`Overlay.show_modal()` / `Overlay.show_modeless()` / `Overlay.show_light_dismiss()` return an `OverlayHandle[T]`.

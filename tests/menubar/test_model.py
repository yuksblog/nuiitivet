...
def test_action_item_normalizes_shortcut() -> None:
    item = MenuBarItem("Open...", on_select=lambda: None, shortcut="Accel+O")
...
def test_submenu_excludes_action_properties() -> None:
    child = MenuBarItem("Child", on_select=lambda: None)
    with pytest.raises(ValueError):
        MenuBarItem("File", submenu=[child], on_select=lambda: None)
    with pytest.raises(ValueError):
        MenuBarItem("File", submenu=[child], shortcut="Accel+F")
    with pytest.raises(ValueError):
        MenuBarItem("File", submenu=[child], checked=Observable(False))
...
def test_resolved_reads_follow_observables() -> None:
    ...
    item = MenuBarItem(label, on_select=lambda: None, enabled=enabled)
...
def test_menu_bar_validates_items_and_keeps_style() -> None:
    ...
    bar = MenuBar([MenuBarItem("File", submenu=[MenuBarItem.quit()])], style=style)

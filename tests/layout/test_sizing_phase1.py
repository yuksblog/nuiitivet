...
def test_fixed_auto_do_not_shrink_in_column():
    ...
    weights = [0, 0, 0]
    usable = 80 - 20
    alloc = Column._allocate_main_sizes(base_sizes, weights, usable)
    assert alloc == [50, 0, 30]


def test_stretch_shrinks_to_zero_if_needed():
    """weight elements should shrink to 0 if space is very limited."""
    _ = Column(
        [
            Container(height=Sizing.fixed(40)),
            Container(height=Sizing.weight()),
            Container(height=Sizing.fixed(40)),
        ],
        gap=10,
        padding=0,
...
def test_stretch_weight_distribution():
    """weight elements should share space proportionally."""
    _ = Column(
        [
            Container(height=Sizing.weight(1)),
            Container(height=Sizing.weight(2)),
            Container(height=Sizing.weight(1)),
        ],
        gap=0,
        padding=0,
...
def test_mixed_fixed_auto_stretch():
    """Mix of fixed, auto, and weight should work correctly."""
    _ = Column(
        [
            Container(height=Sizing.fixed(30)),
            Container(height=Sizing.auto()),
...
def test_row_fixed_auto_do_not_shrink():
    ...
    weights = [0, 0, 0]
    usable = 60
    alloc = Row._allocate_main_sizes(base_sizes, weights, usable)
    assert alloc == [50, 0, 30]


def test_stretch_only_no_overflow():
    """If only weight elements, no overflow should occur."""
    _ = Column(
        [
            Container(height=Sizing.weight()),
            Container(height=Sizing.weight()),
...
def test_zero_weight_stretch_behaves_like_auto():
    """Sizing.weight(0) should be treated as auto (edge case)."""
    dim = Sizing.weight(0)

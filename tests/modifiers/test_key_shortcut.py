...
def _press_accel_s(app: Window) -> bool:
...
def test_foreground_fires_with_nothing_focused() -> None:
    ...
    app = App(Window(content=Column([canvas]))).main_window
...
def test_foreground_fires_while_focus_is_elsewhere() -> None:
    ...
    app = App(Window(content=Column([canvas, toolbar_field]))).main_window
...
def test_foreground_does_not_fire_when_hidden() -> None:
    ...
    app = App(Window(content=Column([panel]))).main_window
...
def test_foreground_does_not_fire_on_a_covered_route() -> None:
    ...
    app = App(Window(content=home)).main_window
    app.root.mount(app)
...
def test_foreground_does_not_fire_on_a_hidden_deck_page() -> None:
    ...
    index = Observable(0)
    app = App(Window(content=Column([Deck(children=[page0, page1], index=index)]))).main_window
    app.root.mount(app)
...
def test_foreground_does_not_fire_inside_a_closed_collapsible() -> None:
    ...
    app = App(Window(content=Column([Collapsible(panel, opened=opened)]))).main_window
    app.root.mount(app)
...
def test_foreground_does_not_fire_inside_a_disabled_clickable() -> None:
    ...
    app = App(Window(content=Column([Clickable(label, disabled=disabled)]))).main_window
    app.root.mount(app)
...
def test_ambiguous_foreground_match_fires_nothing() -> None:
    ...
    app = App(Window(content=Column([a, b]))).main_window
...
def test_focus_scope_routes_to_the_focused_pane_only() -> None:
    ...
    app = App(
        Window(content=Column([_pane(editor_a, lambda: saved.append("a")), _pane(editor_b, lambda: saved.append("b"))]))
    ).main_window
...
def test_focus_scope_does_not_fire_when_focus_is_outside_or_absent() -> None:
    ...
    app = App(Window(content=Column([_pane(inside, lambda: saved.append("pane")), outside]))).main_window
...
def test_focus_scope_innermost_wins() -> None:
    ...
    app = App(Window(content=outer)).main_window
...
def test_focus_scope_wins_over_foreground() -> None:
    ...
    app = App(Window(content=Column([pane, background]))).main_window
...
def test_foreground_wins_over_mount() -> None:
    ...
    app = App(Window(content=Column([app_wide, panel]))).main_window
...
def test_handler_exception_is_contained() -> None:
    ...
    app = App(Window(content=Column([_box().modifier(key_shortcut("Accel+S", on_trigger=boom))]))).main_window
